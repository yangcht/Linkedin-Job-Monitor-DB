...
A small local app that helps you manage multiple LinkedIn Jobs searches, import saved LinkedIn HTML, and track applications locally.

The default sample config seeds the first search setup with this intent:

- keywords: `Azure`, `PowerBI`, `marine`, `logistics`, `RAG`
...
The current design is documented in [docs/architecture.md](/Users/yangcht/Documents/New%20project/docs/architecture.md).
...
On macOS, double-click [Linked Jobs Monitor.command](/Users/yangcht/Documents/New%20project/Linked%20Jobs%20Monitor.command). It starts the local server and opens the web app.
...
Print the LinkedIn search URLs:

```bash
linked-jobs urls
```

Open every configured search in your default browser:

```bash
linked-jobs open
```

Try a polite public-page check and write a Markdown report:

```bash
linked-jobs check
```

Fetch richer detail pages for visible jobs that still need deadline/details:

```bash
linked-jobs enrich-details --limit 10
```
...
```bash
linked-jobs import-html ~/Downloads/linkedin-search.html
```

Reports are written under `reports/`. Jobs are stored in `.job_state/jobs.sqlite3`.
...
- an expandable list view, with one row per job
- search across title, company, location, notes, application status, and description
- filters for status, keyword, and detail-enrichment state
- sorting by posted date, deadline, first seen date, title, or company
...
Saving a job moves it to the saved list. Marking a job not interested hides it and keeps the ID in `.job_state/jobs.sqlite3`, so the same job will not appear again after future checks or imports. Marking a job applied stores the applied date, application status, and notes.

## Notes

- LinkedIn URL filters are not official API contracts and can change.
- The distance value in LinkedIn job URLs is commonly documented as miles. The sample config stores `radius_km = 300` and generates `distance=187`, which is approximately 300 km.
- The official LinkedIn Job Posting API is for approved partners posting jobs to LinkedIn, not for general job-search fetching.
- For a fully managed alert, also create the same searches inside LinkedIn Jobs and turn on LinkedIn's built-in job alerts.
